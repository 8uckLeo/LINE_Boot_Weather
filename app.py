...
access_token = 'qAUUFsbbknJlAslPb1UkpCMygdSWY7DMSVB8BzylE4N2Q58Efo84fJfqKTeSsDQPHcq4KgovworQH8iiO5JcHf3Z1RbhArOHVoGALvBvzrwXOudC+fX9/UsVWnhM8P9w4rcWKyij3QB0RS8p/LipWwdB04t89/1O/w1cDnyilFU='
channel_secret = 'f76efe0558f9ec3aad9cf61453a80f15'

# 必須放上自己的Channel Access Token
line_bot_api = LineBotApi('qAUUFsbbknJlAslPb1UkpCMygdSWY7DMSVB8BzylE4N2Q58Efo84fJfqKTeSsDQPHcq4KgovworQH8iiO5JcHf3Z1RbhArOHVoGALvBvzrwXOudC+fX9/UsVWnhM8P9w4rcWKyij3QB0RS8p/LipWwdB04t89/1O/w1cDnyilFU=')
# 必須放上自己的Channel Secret
handler = WebhookHandler('f76efe0558f9ec3aad9cf61453a80f15')

# 地震資訊函式
def earth_quake():
    msg = ['找不到地震資訊','https://example.com/demo.jpg']            # 預設回傳的訊息
    try:
        code = 'CWB-84D2CC22-D0E5-442A-AEF5-D4AA97FD9CC7'
        ...
        for i in eq:
            ...
            msg = [f'{loc}，芮氏規模 {val} ，深度 {dep} 公里，發生時間 {eq_time}。', img]
    ...
    try:
        ...
        for i in city_list:
            if i in address: # 如果地址裡包含縣市名稱的 key，就直接使用對應的內容
                ...
                num_list = city_list[i]
                num_list = [int(num) for num in num_list if num != '']
                aqi_val = round(statistics.mean(num_list),0)  # 計算平均數值，如果找不到鄉鎮區域，就使用縣市的平均值
        ...
        if 'message' in json_data['events'][0]:
            ...
            if json_data['events'][0]['message']['type'] == 'text':
                ...
                if text == '雷達' or text == '雷達回波':
                    ...
                    reply_message("請選擇下面的主選單來得到資訊。", reply_token, access_token)    # 如果是一般文字，直接回覆同樣的文字
    except:
        print('error')
    return 'OK'
